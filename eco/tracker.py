...
class ECOTracker:
    ...
    def init(self, frame, bbox, total_frame=np.inf):
        ...
        for idx, feature in enumerate(features):
            if feature['fname'] == 'cn' or feature['fname'] == 'ic':
                ...
                if netname == 'resnet50':
                    self._features.append(MaskRCNN(self._img_sample_sz))
        ...
        if cnn_feature_idx >= 0:
            # self._img_sample_sz = self._features[cnn_feature_idx].sample_sz
    ...
    def update(self, frame, train=True, vis=False, metric=None):
        ...
        for _ in range(config.refinement_iterations):
            ...
            if not np.allclose(old_pos, pos):
                ...
                sample_pos = mround(pos)    # np.round()
                ...
                # scale estimation by mask
                if metric:
                    start1 = time.time()
                mask = self._features[0].get_mask(translation_vec, target_sz=self._target_sz,
                                                  sample_sz=self._img_sample_sz, output_sz=self._img_sample_sz,
                                                  scale=self._current_scale_factor, metric=metric)
                if metric:
                    metric.mask['total'] += time.time() - start1
