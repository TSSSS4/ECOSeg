...
from seg.structures.backbone import build_resnet_fpn_backbone, build_resnet_backbone
from seg.structures.roi_head import build_roi_heads
from seg.utils.bounding_box import BoxList
from seg.utils.inference import Masker
...
def get_sample_patch(im, pos, base_sample_sz, output_sz, scales=[1]):
    """
    Get multiscale image patches form a certain position of the image.
    :param im: image
    :param pos: patch center
    :param base_sample_sz: base sample size
    :param output_sz: output patch size
    :param scales: sample scales
    :return: image patches in the form of [num, c, h, w]
    """
    pos = np.floor(pos)
    patches = []
    for scale in scales:
        sample_sz = np.maximum(mround(base_sample_sz * scale), 1)
        ...
        patches.append(np.expand_dims(np.array(im_patch), axis=0))
...
def feature_normalization(features):
    ...
    :param features: input features[num, c, h, w]
    :return: normalized features[h, w, c, num]
    """
    # convert tensor[s,c,h,w] to numpy[h,w,c,s]
    if torch.is_tensor(features):
        x = features.data.cpu().numpy().transpose((2, 3, 1, 0))
    else:
        x = features.transpose((2, 3, 1, 0))
    # normalize
...
class MaskRCNN:
    def __init__(self, sample_sz):
        # parameters
        self._compressed_dim = config.cnn_params['compressed_dim']
        self._cell_size = [4, 32]
        self.penalty = [0., 0.]
        self.min_cell_size = np.min(self._cell_size)

        self.num_dim = [256, 256]
        self.data_sz = []       # backbone network output size(two tracking layer feature)
        self.sample_sz = []     # backbone network input size
        ...
        self.masker = Masker(config.mask_thresh, config.mask_padding)
    ...
    def init_size(self, img_sample_sz):
        ...
        feat2_shape = np.ceil(img_sample_sz / total_stride)
        desired_sz = feat2_shape + 1 + feat2_shape % 2
        img_sample_sz = (desired_sz * total_stride).astype(np.int32)
        self.num_dim = config.num_dims
        self.sample_sz = img_sample_sz.astype(np.int32)
    ...
    def _load_weights(self, model, file_path):
        ...
        for key in model_state_dict.keys():
            if loaded_state_dict[key] is not None:
    ...
    def proposals(self, translation, target_sz, sample_sz, output_sz, scale):
        """
        Determine target's bounding box(proposal of rpn) in current sample patch.
        :param translation: target translation of position from sample patch center.
        :param target_sz: current target size.
        :param sample_sz: current sample size.
        :param output_sz: resized sample size, which is the input of backbone network.
        :param scale: current sampling scale.
        :return: A bounding box [xmin, ymin, xmax, ymax]
        """
        rect = []
        # calculate center translation and corners' translation
        translation = np.array(translation)
        sample_sz = np.array(sample_sz) * scale
        output_sz = np.array(output_sz)
        center = sample_sz / 2 + translation
        rect.extend(center - target_sz / 2)
        rect.extend(center + target_sz / 2)
        # rescale
        rect = np.repeat(output_sz / sample_sz, 2, axis=0) * rect
        # restrict value, convert [y1,x1,y2,x2] -> [x1,y1,x2,y2]
        rect = np.array([max(rect[1], 0), max(rect[0], 0), min(rect[3], output_sz[1]), min(rect[2], output_sz[0])])

        proposals = []
        boxlist = torch.as_tensor(rect).unsqueeze(0)
        if config.use_gpu:
            boxlist = boxlist.cuda()
        boxlist = BoxList(boxlist, self.sample_sz[-1::-1], mode='xyxy')
    ...
    def get_features(self, img, pos, sample_sz, scales):
        ...
        if not isinstance(scales, list) and not isinstance(scales, np.ndarray):
            scales = [scales]
        patches = get_sample_patch(img, pos, sample_sz, sample_sz, scales)          # [num, h, w, c]
        self.patches = patches
        patches = [self.transform(patch).unsqueeze(0).cuda() for patch in patches]
        patches = torch.cat(patches, dim=0)
        # patch = patch[None]
        self.features = self.backbone(patches)
        f1 = feature_normalization(self.features[config.feature_idx[0]])    # x4, [h,w,c,num]
        f2 = feature_normalization(self.features[config.feature_idx[1]])    # x16
        return f1, f2

    def get_mask(self, translation, target_sz, sample_sz, output_sz, scale, metric=None):
        if metric:
            start = time.time()
        proposals = self.proposals(translation, target_sz, sample_sz, output_sz, scale)
        if metric:
            metric.mask['proposal'] += time.time() - start
            start = time.time()
        mask_features = [self.features[idx] for idx in config.mask_feature_idx]
        if metric:
            metric.mask['feature'] += time.time() - start
            start = time.time()
        x, prediction, detector_losses = self.roi_heads(mask_features, proposals, None, metric=metric)
        if metric:
            metric.mask['roihead'] += time.time() - start
            start = time.time()
        prediction = [o.to(self.cpu_device) for o in prediction]
        prediction = prediction[0]
        prediction = prediction.resize(tuple(self.sample_sz[-1::-1]))
        if metric:
            metric.mask['prediction'] += time.time() - start
            start = time.time()
        masks = prediction.get_field("mask")
        masks = self.masker([masks], [prediction])[0]
        if metric:
            metric.mask['masker'] += time.time() - start
            start = time.time()
        # prediction.add_field("mask", masks)
        # result = self.overlay_mask(self.patches[0], prediction)
        ...
        if mask.size > 0:
            ...
            rect = cv2.minAreaRect(idx)     # xy, wh, angle
            box = cv2.boxPoints(rect)       # x0y0 ~ x3y3
            box = np.int0(box)
