...
class Metric:
    def __init__(self):
        ...
        self.feat = 0
        self.update = 0
        self.mask = {'total': 0, 'proposal': 0, 'feature':0,
                     'roihead':0, 'head_box':0, 'head_mask':0,
                     'prediction':0, 'masker':0, 'minRect':0}

    def print(self, total_frame):
        print('trans:', self.trans / total_frame)
        print('feat:', self.feat / total_frame)
...
def main(video_dir):
    ...
    for idx, frame in enumerate(frames):
        if idx == 0:
            ...
            start = time.time()
            metric, bbox = tracker.update(frame, True, vis, metric=metric)
            total_time += time.time() - start
            total_frame += 1
        else:   # last frame
            metric, bbox = tracker.update(frame, False, vis, metric=metric)
    ...
    metric.print(total_frame)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--video_dir', type=str, default='/home/cxq/study/objectTracking/benchmark/OTB100/Basketball/')
